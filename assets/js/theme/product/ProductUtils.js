...
export default class ProductUtils {
  ...
  init(context) {
    ...
    if ($('[data-review-messages]').length) {
      $('html, body').animate({scrollTop: $('[data-review-messages]').offset().top - 30}, 1000);
    }
  }
  ...
  _bindProductOptionChange() {
    utils.hooks.on('product-option-change', (event, changedOption) => {
      ...
      utils.api.productAttributes.optionChange(this.productId, $form.serialize(), (err, response) => {
        ...
        // If our form data doesn't include the product-options-count with a positive value, if there is a form,
        // return
  ...
  _bindCartAdd() {
    utils.hooks.on('cart-item-add', (event, form) => {
      // Do not do AJAX if browser doesn't support FormData or if option set to off
      if (window.FormData === undefined || this.context.disableProductAjax) { return; }
  ...
  _initVids(tabId) {
    if (tabId === '#videos' && !this.fitVidsInitialized) {
      $('.tab-product-video-wrapper').fitVids();
